...
8. [파일 구조](#8-파일-구조)
9. [알려진 한계](#9-알려진-한계)
10. [Face_Analysis/shape — 배포용 추론 전용 복사본](#10-face_analysisshape--배포용-추론-전용-복사본)
...
## 10. Face_Analysis/shape — 배포용 추론 전용 복사본

`train_pipeline3/`의 학습 결과물(`feature_extractor.py`, `shape_classification.py`,
`face_landmark_detection.py`, `best_model.pkl`)을 `Face_Analysis/shape/`에 추론
전용으로 복사해 배포한다. `feature_extractor.py`는 동일하지만, 나머지 두 파일은
학습/시각화/테스트 코드를 제거한 만큼 동작이 일부 다르다.

| | `train_pipeline3/` | `Face_Analysis/shape/` |
|---|---|---|
| `feature_extractor.py` | 동일 | 동일 |
| `shape_classification.py` 반환값 | `candidates` 키 포함, `__main__`에 테스트셋 평가(Top-1/Top-2, confusion matrix) 코드 있음 | `candidates` 키 없음 (`face_shape_2`/`face_shape_2_en`만 사용), `__main__` 평가 코드 제거 |
| `face_landmark_detection.py` | `detect_landmarks()` — OpenCV 시각화 + 결과 이미지 저장 + CLI(`python face_landmark_detection.py 입력.jpg 출력.jpg`) 포함 | `detect_landmarks()` 제거 — `compute_ratios()`/`classify_samjeong()`/`detect_hairline()` 등 계산 함수만 남음, CLI 진입점 없음 |

재배포 시 위 표의 차이를 의도적으로 유지해야 한다 — `Face_Analysis/shape/`는
`diagnose.py`에서 호출되는 라이브러리 모듈이라 시각화·평가 코드가 필요 없다.
모델(`best_model.pkl`)이나 `feature_extractor.py`를 갱신할 때는 두 위치 모두
동기화해야 하며, `shape_classification.py`/`face_landmark_detection.py`는 학습용
코드를 다시 섞어넣지 않도록 위 표 기준으로 diff를 확인할 것.
